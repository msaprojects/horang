...
class StorageNonAktifDummy extends StatefulWidget {
  // String token;
...
class _SearchListViewExampleState extends State<StorageNonAktifDummy> {
  ...
  init() async {
    print('masuk init');
    var storage = await _apiService.listMystorageNew(access_token, query);
    ...
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: FutureBuilder(
              future: _apiService.listMystorageNew(access_token, query),
  ...
  Future searchmystorage(String query) async => debounce(() async {
        ...
        final storage = await _apiService.listMystorageNew(access_token, query);
        if (!mounted) return;
        setState(() {
